...
import EditConsularFileDialog from "../components/ConsularFileForm/EditConsularFileDialog";
...
export default function Dashboard() {
  ...
  // State for records, search, edit, pagination
  ...
  // Fetch consular files
  const fetchConsularFiles = () => {
    setLoading(true);
    API.get("/consularFile", {
      params: {
        page: consPage,
        limit: CONS_LIMIT,
        search: consSearch,
  ...
  const handleDeleteCons = async (id) => {
    if (!window.confirm(t("deleteConsularFileConfirm") || "Delete this consular file?")) return;
    await API.delete(`/consularFile/${id}`);
    fetchConsularFiles();
  };
  ...
  return (
    ...
    >
        ...
        >
          <Stack direction={isMobile ? "column" : "row"} spacing={2} alignItems={isMobile ? "flex-start" : "center"}>
            <Avatar sx={{
              ...
              width: 56, height: 56, fontSize: 32, fontWeight: 700, letterSpacing: 1,
            }}>
              {(user?.name || "U")[0]}
            </Avatar>
            <Box flex={1}>
            ...
            <Stack direction="row" spacing={2} mt={isMobile ? 2 : 0}>
              ...
              <Button
                ...
                sx={{ textTransform: "none", fontWeight: 600, background: "linear-gradient(90deg, #b10056 0%, #f83392 100%)" }}
              >
                {t("consularFileForm")}
              </Button>
              <Button
      ...
      <Fade in timeout={600}>
        <Paper elevation={2} sx={{ p: isMobile ? 1.5 : 3, mb: 4, borderRadius: 3, maxWidth: 800, mx: "auto" }}>
          <Typography variant="h6" mb={2} color="#b10056" fontWeight={700} letterSpacing={0.5}>
                ...
                >
                  <Box flex={1}>
                    <Stack direction="row" alignItems="center" spacing={1}>
                      <Chip label={reg.fileNumber || "-"} sx={{ bgcolor: "#f6d6e8", color: "#b10056", fontWeight: 600, fontSize: "0.95em" }}/>
                      <Typography fontWeight={600} sx={{ ml: 1 }}>
                        {reg.fullName || "-"}
                      </Typography>
                      <Typography color="text.secondary" fontSize="0.97em" sx={{ ml: 2 }}>
                    ...
                    {reg.passportPhoto && (
                      <Avatar
                        ...
                        alt={t("photo")}
                        variant="rounded"
                        sx={{ mt: 1, width: 46, height: 46, border: "2px solid #f83392" }}
                      />
                    )}
                  </Box>
      ...
      {/* CONSULAR FILE RECORDS */}
      <Fade in timeout={900}>
        <Paper elevation={2} sx={{ p: isMobile ? 1.5 : 3, borderRadius: 3, maxWidth: 800, mx: "auto" }}>
          <Typography variant="h6" mb={2} color="#b10056" fontWeight={700} letterSpacing={0.5}>
          ...
          <Stack direction={isMobile ? "column" : "row"} spacing={2} sx={{ mb: 2 }}>
            ...
            <TextField
              ...
              type="date"
              value={consDateFrom}
              onChange={(e) => setConsDateFrom(e.target.value)}
              InputLabelProps={{ shrink: true }}
              sx={{ width: isMobile ? "100%" : 150 }}
            />
            <TextField
              ...
              type="date"
              value={consDateTo}
              onChange={(e) => setConsDateTo(e.target.value)}
              InputLabelProps={{ shrink: true }}
              sx={{ width: isMobile ? "100%" : 150 }}
            />
            <Button
            ...
            <Box component="ul" sx={{ pl: 2, m: 0 }}>
              {consularFiles.map((file, idx) => (
                ...
                >
                  <Box flex={1}>
                    <Stack direction="row" alignItems="center" spacing={1}>
                      <Chip label={file.fileNumber || "-"} sx={{ bgcolor: "#f6d6e8", color: "#b10056", fontWeight: 600, fontSize: "0.95em" }}/>
                      <Typography fontWeight={600} sx={{ ml: 1 }}>
                  ...
                  <Stack direction="row" spacing={1}>
                    <Tooltip title={t("edit") || "Edit"}>
                      <IconButton
                        size="small"
                        color="primary"
                        onClick={() => setEditCons(file)}
                        aria-label="edit consular file"
                      >
                        <EditIcon />
                      </IconButton>
                    </Tooltip>
                    <Tooltip title={t("delete") || "Delete"}>
                      <IconButton
                        size="small"
                        color="error"
                        onClick={() => handleDeleteCons(file._id)}
                        aria-label="delete consular file"
                      >
                        <DeleteIcon />
                      </IconButton>
                    </Tooltip>
